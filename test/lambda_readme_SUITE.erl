...
basic(Config) when is_list(Config) ->
    ...
    {module, calc} = peer:call(AuthPeer, code, load_binary, [calc, nofile, Code]),
    {ok, StartedApps} = peer:call(AuthPeer, application, ensure_all_started, [lambda]),
    %% Server: publish calc
    {ok, _Srv} = peer:call(AuthPeer, lambda, publish, [calc, #{capacity => 3}]),
    %% Client: discover calc (using epmd)
    {ok, ClientPeer, _Client} = ?CT_PEER(#{connection => standard_io, name => peer:random_name(), args => VmArgs}),
    {ok, StartedApps} = peer:call(ClientPeer, application, ensure_all_started, [lambda]),
    {ok, Plb} = peer:call(ClientPeer, lambda, discover, [calc, #{capacity => 10}]),
    ...
    {ok, Srv2Peer, _Srv2} = ?CT_PEER(#{connection => standard_io, name => peer:random_name(), args => VmArgs}),
    {ok, StartedApps} = peer:call(Srv2Peer, application, ensure_all_started, [lambda]),
    {module, calc} = peer:call(Srv2Peer, code, load_binary, [calc, nofile, Code]),
    {ok, _Srv2Srv} = peer:call(Srv2Peer, lambda, publish, [calc, #{capacity => 3}]),
    %% wait for Client Plb to find new server and update capacity
    wait_capacity_change(ClientPeer, Plb),
    %% ensure that client got more capacity: originally 3, 1 request executed,
    %%  2 more left, and 3 more connected = total of 5
    ?assertEqual(5, peer:call(ClientPeer, lambda_plb, capacity, [Plb])),
    %% Shutdown
    lambda_async:pmap([{peer, stop, [S]} || S <- [AuthPeer, Srv2Peer]]),
    wait_capacity_change(ClientPeer, Plb),
    %% Capacity goes down to zero
    ?assertEqual(0, peer:call(ClientPeer, lambda_plb, capacity, [Plb])),
    %% stop PLB, which should make 'calc' module unload
    ok = peer:call(ClientPeer, gen_server, stop, [Plb]),
    ?assertEqual(non_existing, peer:call(ClientPeer, code, which, [calc])),
