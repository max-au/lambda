...
proper(Config) when is_list(Config) ->
    case proper:quickcheck(prop_self_healing(),
...
precondition(#cluster_state{auth = Auth}, {call, ?MODULE, start_broker, []}) ->
    length(Auth) < ?MAX_AUTHORITY;

precondition(#cluster_state{brokers = Brokers}, {call, ?MODULE, start_authority, []}) ->
    length(Brokers) < ?MAX_BROKERS;

precondition(#cluster_state{auth = Auth, brokers = Brokers}, {call, gen_server, stop, [Pid]}) ->
...
%%  * net splits
command(#cluster_state{auth = Auth, brokers = Brokers}) ->
    %% nodes starting (authority/broker)
    AuthorityStart = {1, {call, ?MODULE, start_authority, []}},
    BrokerStart = {5, {call, ?MODULE, start_broker, []}},
    ...
    Choices = [AuthorityStart, BrokerStart | Stop],
    proper_types:frequency(Choices).

next_state(#cluster_state{auth = Auth} = State, Res, {call, ?MODULE, start_authority, []}) ->
    NewAuth = {call, erlang, element, [2, Res]},
    State#cluster_state{auth = [NewAuth | Auth]};

next_state(#cluster_state{brokers = Brokers} = State, Res, {call, ?MODULE, start_broker, []}) ->
    NewBroker = {call, erlang, element, [2, Res]},
    State#cluster_state{brokers = [NewBroker | Brokers]};
...
next_state(#cluster_state{auth = Auth, brokers = Brokers} = State, _Res, {call, peer, stop, [Pid]}) ->
    ...
    State#cluster_state{auth = lists:delete(Pid, Auth), brokers = lists:delete(Pid, Brokers)}.
